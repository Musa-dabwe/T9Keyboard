 ...
 * - Tapping a tab jumps to that category's first section header
 */
class EmojiPickerView @JvmOverloads constructor(
    ...
    private fun setupViews() {
        ...
        // Category tabs recycler (fills remaining space)
        ...
        // GridLayoutManager with span of COLS, headers span full width
        val glm = GridLayoutManager(context, COLS)
        glm.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                return when (flatList[position]) {
                    is ListItem.Header -> COLS
                    is ListItem.EmojiRow -> 1 // each cell in the row = 1 col
                }
            }
        }
        // But we're using rows not individual cells. Switch to LinearLayoutManager with custom row views.
    ...
    inner class TabsAdapter(
        ...
        override fun onBindViewHolder(holder: TabVH, position: Int) {
            ...
            if (isSelected) {
                holder.tv.setPadding(0, dpToPx(4), 0, dpToPx(2))
            } else {
                holder.tv.setPadding(0, dpToPx(4), 0, dpToPx(4))
            }
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            when (val item = items[position]) {
                is ListItem.Header -> {
                    ...
                    // Format: SUBCATEGORY (e.g. "face-smiling" -> "FACE SMILING")
